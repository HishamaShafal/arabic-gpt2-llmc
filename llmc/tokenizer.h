...
void safe_printf(const char *piece) {
    ...
    // دعم طباعة بايتات UTF-8 الخاصة باللغة العربية دون التسبب بظهور رموز غريبة
    // نسكب فقط رموز التحكم غير الطباعية الأساسية (كالـ Control Characters)
    unsigned char first_byte = piece[0];
    if (first_byte < 32 && first_byte != '\n' && first_byte != '\t' && first_byte != '\r') {
        return; 
    }
    printf("%s", piece);
}
